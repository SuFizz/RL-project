...
#DONT READ. THIS COMMENT WILL CONFUSE <==> I realize that irrespective of whatever number of ACKs I get, I am happy if I have maximum rate. This just goes onto say that I need some kind of transmission happening. So, I define 2 states - [[No ACK's and NACK's]] and [[At least 1 ACK]] ===> WRONG CONCLUSION
	...
	for iterator in range(100):
		...
		RpC_max = numpy.array([numpy.argmax(RpC[:,i]) for i in range(M)])
		alpha = 0.8
		gamma = 0.9
	...
	#	maxe = -10
		_states = M+1
		Q = numpy.random.uniform(20,5,(_states,N,M))
...
#			print t
	...
	#		if()
			...
			interm = Q[state];
			action_f = numpy.array([numpy.argmax(interm[:,i]) for i in range(M)])
...
#				_action_ = numpy.random.randint(0,N,M);
	#			print "random",'\n',t
			cnt = len(numpy.where((_action_ == RpC_max) == True)[0])/M
			percent_optimal[iterator].append(cnt)
	...
	#		print _action#,Reward
			ee = numpy.array([numpy.argmax(interm[:,i]) - numpy.argmax(RpC[:,i]) for i in range(M)])
	#		xxx.append(len(numpy.where(ee==0)[0]))
	#		x_es.append(_action)
